...
contract Streets is IERC721Receiver {
    ...
    function stake(uint256 tokenId) external {
        // @audit check if oneShotContract.ownerOf(tokenId) == msg.sender
        stakes[tokenId] = Stake(block.timestamp, msg.sender);
        emit Staked(msg.sender, tokenId, block.timestamp);
        oneShotContract.transferFrom(msg.sender, address(this), tokenId);
    }

    // @audit NFT can be stacked again (even if it's not wining any stats anymore)
    ...
    function unstake(uint256 tokenId) external {
        ...
        uint256 daysStaked = stakedDuration / 1 days; // @audit precision loss ?

        // @audit wrong comment ?
        // Assuming RapBattle contract has a function to update metadata properties
        // @audit we don't need the old rapperStats.
        ...
        //@audit not gas efficient, should mint once only: credContract.mint(msg.sender, daysStaked);
        ...
        // @audit transfer is better (no need for trasnferFrom)
    ...
    // @audit only needed for safeTransfer, safeTransferFrom...
