...
contract RapBattle {
    ...
    function goOnStageOrBattle(uint256 _tokenId, uint256 _credBet) external {
        if (defender == address(0)) {
            ...
            // @audit emit after transferFrom: what if someone tryied to deposit someone else's NFT
            emit OnStage(msg.sender, _tokenId, _credBet);

            oneShotNft.transferFrom(msg.sender, address(this), _tokenId);
            credToken.transferFrom(msg.sender, address(this), _credBet);
        } else {
            // @audit no regrets: delete the comment below
            // credToken.transferFrom(msg.sender, address(this), _credBet);
    ...
    function _battle(uint256 _tokenId, uint256 _credBet) internal {
        ...
        // @audit not true random, Attacker can exploited
        uint256 random =
        ...
        // Can have a different Annouced winner (Event) and actual winner
